...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber[] partialProducts(NaturalNumber[] nnArray) {
        ...
        NaturalNumber[] partial = new NaturalNumber2[nnArray.length];
        for (int i = 1; i < partial.length + 1; i++) {
            NaturalNumber product = new NaturalNumber2(1);
            for (int j = 0; j < i; j++) {
                product.multiply(nnArray[j]);
            }
            partial[i - 1] = new NaturalNumber2(product);
        }
    ...
    public static void main(String[] args) {
        ...
        NaturalNumber[] product2 = partialProducts(array);
        out.println(Arrays.toString(product2));
