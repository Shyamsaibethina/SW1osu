...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        //Gets the substring of str1 without the overlap portion
        String result = str1.substring(0, str1.length() - overlap);

        //returns str1 without the overlap portion added with str2
        return result + str2;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        /*
         * If str is a substring of a string in the set, then we end the method
         * and return nothing
         */
        for (String s : strSet) {
            if (s.indexOf(str) != -1) {
        ...
        /*
         * If there is no string within the set that has a substring equal to
         * str, then we add str to the set
         */
        strSet.add(str);

        //Creates a new temporary set
        Set<String> temp = strSet.newInstance();

        //Clears strSet and transfers elements to temporary set for later use
        temp.transferFrom(strSet);

        /*
         * Goes through each element in the temporary set and makes sure to add
         * str to the empty strSet. If it is already added, it then checks if
         * str has a substring equal to each element in s, and if not, it adds
         * it to strSet.
         */
        for (String s : temp) {
            if (s.equals(str)) {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //Creates empty set to add lines to
        Set<String> result = new Set1L<>();

        //Checks if the input is not at the end
        while (!input.atEOS()) {
            //if input is not at the end, then we get the next line
            String nextLine = input.nextLine();
            //adds the next line to the resulting set if there are no substrings
            addToSetAvoidingSubstrings(result, nextLine);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
         ...
         * separator. Then prints out the resulting string
         */
